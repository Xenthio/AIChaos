...
if SERVER then
    ...
    function DownloadAndMountWorkshopAddon(workshopId, callback)
        ...
        steamworks.FileInfo(workshopId, function(result)
            if not result or not result.fileid then
                print("[AI Workshop] Failed to get addon info for " .. workshopId)
            ...
            steamworks.Download(workshopId, true, function(path)
                ...
                if success then
                    print("[AI Workshop] Successfully mounted addon " .. workshopId .. " from " .. path)
    ...
    function DownloadAndGetWorkshopAssets(workshopId, callback)
        DownloadAndMountWorkshopAddon(workshopId, function(success, path)
            ...
            -- Get all files from the mounted addon
            local files, dirs = file.Find("models/*", "GAME")
            for _, f in ipairs(files or {}) do
                if string.EndsWith(f, ".mdl") then
                    table.insert(assets.models, "models/" .. f)
                end
            end
    ...
    -- callback(success, entity) - called with the spawned entity or nil
    function DownloadAndSpawnWorkshopModel(workshopId, callback)
        DownloadAndGetWorkshopAssets(workshopId, function(success, assets)
            ...
            print("[AI Workshop] Spawned model: " .. modelToSpawn)
            if callback then callback(true, ent) end
        end)
    end
